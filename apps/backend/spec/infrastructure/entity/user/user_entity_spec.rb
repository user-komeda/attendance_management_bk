...
RSpec.describe Infrastructure::Entity::User::UserEntity do
  ...
  it 'builds domain entity from struct' do
    src = Struct.new(:id, :first_name, :last_name, :email, :session_version).new('u-1', 'Ken', 'Tanaka',
                                                                                 'ken@example.com', 1)
    domain = described_class.struct_to_domain(struct: src)
  ...
  it 'generates uuid when id is nil on build from domain' do
    ...
    infra = described_class.build_from_domain_entity(user_entity: domain)
