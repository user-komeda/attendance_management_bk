  ...
  module Entity
    module User
      class UserEntity < Infrastructure::Entity::BaseEntity
        ...
        attribute :session_version, ROM::Types::Integer
        ...
        def to_domain
          ::Domain::Entity::User::UserEntity.build_with_id(id: id, first_name: first_name, last_name: last_name,
                                                           email: email, session_version: session_version)
        end

        def to_domain_with_auth_user; end

        # @rbs (struct: untyped) -> ::Domain::Entity::User::UserEntity
        def self.struct_to_domain(struct:)
          user_entity = new(
            ...
            email: struct.email,
            session_version: struct.session_version
          )
          user_entity.to_domain
        end

        # @rbs (struct: untyped) -> {user_entity: Domain::Entity::User::UserEntity, auth_user_entity: Domain::Entity::Auth::AuthUserEntity}
        def self.struct_to_domain_with_auth_user(struct:)
          user_entity = struct_to_domain(struct: struct)
