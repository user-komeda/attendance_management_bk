...
RSpec.describe 'User API integration', type: :request do
  ...
  before do
    ...
    # ContainerHelper縺ｮresolve繝｡繧ｽ繝・ラ繧偵せ繧ｿ繝門喧
  ...
  it 'returns list of created users' do
    created_users = [
      @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Taro', last_name: 'Yamada', email: 'taro@example.com')),
      @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Hanako', last_name: 'Suzuki', email: 'hanako@example.com'))
    ]
  ...
  it 'returns 409 when email is duplicated' do
    @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Existing', last_name: 'User', email: 'duplicate@example.com'))
  ...
  it 'returns 200 with user details' do
    user = @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Test', last_name: 'User', email: 'test@example.com'))
  ...
  it 'returns 204 and updates data' do
    user = @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Old', last_name: 'Name', email: 'old@example.com'))
  ...
  it 'updates only provided fields (partial update)' do
    user = @test_repo.create(::Domain::Entity::User::UserEntity.build(first_name: 'Original', last_name: 'User', email: 'original@example.com'))
